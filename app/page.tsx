...
export default function Home(){
 ...
 async function read(f:File){
  ...
  try{
   ...
   for(let i=1;i<=pdf.numPages;i++){
    ...
    all+=(c.items as any[]).map(x=>x.str||"").join(" ")+" ";
   }
   ...
   setError(e?.message||"Não foi possível ler o PDF.")
  }finally{
   setBusy(false)
  }
 }
 ...
 const download=()=>{
  ...
  const rows=[
   ...
   ["Chave de acesso",data.key]
  ];
  const csv=rows.map(r=>r.map(v=>`"${(v||"Não identificado").replaceAll('"','""')}"`).join(";")).join("\n");
  const a=document.createElement("a");
  a.href=URL.createObjectURL(new Blob(["\ufeff"+csv],{type:"text/csv"}));
  a.download="mandae-nota.csv";
  a.click()
 };

 return <main>
  <header><div className="brand"><span>M</span><div><b>MANDAÊ</b><small>LEITOR DE NOTAS</small></div></div></header>
